...
function doPost(e) {
  try {
    ...
    if(data.action === "register") {
      return jsonResponse(register(data));
    }
...
function getDashboardAnalytics() {
  ...
  const kategoriRows = getDataRows(
    getSheet(SHEET_KATEGORI)
  );

  const produkRows = getDataRows(
    getSheet(SHEET_PRODUK)
  );

  const reviewRows = getDataRows(
    getSheet(SHEET_REVIEW)
  );
  ...
  const statistikKategori =
    getStatistikKategori(
      produkRows,
      kategoriRows
    );

  const produkTerpopuler =
    getProdukTerpopuler(
      produkRows,
      reviewRows
    );
...
function getStatistikKategori(
  produkRows,
  kategoriRows
) {

  return kategoriRows.map(
    kategori => {
...
function getProdukTerpopuler(
  produkRows,
  reviewRows
) {

  const result = produkRows.map(
    (produk) => {
...
function register(data) {
  ...
  const existingUser = rows.find((row) => {
    return String(row[2]).toLowerCase() === email.toLowerCase();
  });
